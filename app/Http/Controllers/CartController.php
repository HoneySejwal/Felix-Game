...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request){
        ...
        request()->session()->flash('success',__('common.product_added'));
        return back();       
    }  
 ...
 //return $request;
    ...
    $cartp = Cart::where('user_id', $user_id)->where('order_id',null)->pluck('product_id')->first();
       ...
       //return $cartp;
       if ($cartp > 0) {
       if($cartp==1 && $product->id>1)
            {
    return back()->with('error',__('common.purchase_message'));
            }
       if($cartp>1 && $product->id==1)
            {
        return back()->with('error',__('common.cannot_add_product'));
            }
       }
        //return $product;
        if($request->hours>0)
...
//$already_cart='';
    ...
    public function cartDelete(Request $request){
        ...
        if ($cart) {
            $cart->delete();
            request()->session()->flash('success',__('common.cart_removed'));
            return back();  
        }
        request()->session()->flash('error',__('common.generic_error'));
        return back();       
    }     
        ...
        if($request->quant){
            ...
            foreach ($request->quant as $k=>$quant) {
                ...
                if($quant > 0 && $cart) {
                    ...
                    //$cart->save();
                    $success = __('common.cart_updated');
                }else{
...
public function trainingdelete(Request $request){
        ...
        Cart::where('id', $cart->id)->update([
            ...
            'amount_hk' => $product_detail->price
                    ...
                    //$cart->save();
                    $success = __('common.training_removed');
...
public function pointsredeem(Request $request){
    ...
    if (!$cart) {
    return back()->with('error', __('common.cart_not_found'));
}
...
$cart->update([
    ...
    public function checkout(Request $request){
       ...
       $product_id_collection = $cart->pluck('product_id');
        ...
        if ($product_id_collection->contains(1)) {
      $cart = Cart::where('user_id', $user_id)->where('order_id',null)->where('product_id', '>', 1)->delete();
