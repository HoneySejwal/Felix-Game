...
@section('main-content')
  ...
  <section class="page-cntnt pt-5 pb-5">
        ...
        $points=DB::table('users')->where('id',auth()->user()->id)->pluck('points')->first();
                ...
                @endphp
   <p>
       @auth<span class="text-start">{{ __('common.hi') }}, <b style="font-size:24px;color:#b6f1d5 ;">{{ Auth::user()->name }}</b>  ( <a href="{{ route('user.logout') }}"> {{ __('common.logout') }} </a> )</span>@endauth
       <span class="text-end" style="float: right;">{{ __('common.available_credits') }}: <b style="font-size:24px;color:#b6f1d5 ;">{{$points}} </b></span>
                    </p>           
                </div>
    ...
    <button class="nav-link" id="pointshistory-tab" data-bs-toggle="pill" data-bs-target="#pointshistory" type="button" role="tab" aria-controls="pointshistory" aria-selected="false">{{ __('common.points_history') }}</button>       
                ...
                <form>
                           <div class="row">
                                      ...
                                      <div class="text-center">                                     
                                        <button type="submit" class="default-button mt-3 mt-sm-0"> Continue </button>
                                  </div>
                </form>
              </div>
            </div>
                                   ...
                                   <div class="row">
                                        <div class="col-xl-6 col-lg-6 col-md-12">                                
                                            ...
                                            <div class="contact-three__input-box mb-2">
                                               <input type="password" name="current_password" class="form-control" oninput="clearError(this)">
                                       ...
                                       <div class="col-xl-6 col-lg-6 col-md-12"> 
                                            ...
                                            <div class="contact-three__input-box mb-2">
                                              <input type="password" name="new_password" class="form-control" oninput="clearError(this)">
                                     ...
                                     <div class="row align-items-center"> 
                                       <div class="col-xl-6 col-lg-6 col-md-12"> 
                                          <h4 class="contact-three__input-title">{{ __('common.new_password_confirm') }}</h4>
                                            <div class="contact-three__input-box mb-2">
                                                <input type="password" name="new_confirm_password" class="form-control" oninput="clearError(this)">
                                       ...
                                       <div class="col-xl-6 col-lg-6 col-md-12"> 
                                            <div class="mt-4 text-end">                                                
                                                <button type="submit" class="default-button"><span>{{ __('common.change_password') }}</span></button>
                                             </div>   
                </div>
</div>   
                </form>
...
<a class="btn btn-secondary float-end" href="{{route('user')}}"> <span> {{ __('common.back') }} </span> </a>
          ...
          <div class="tab-pane accordion-item" id="pointshistory" role="tabpanel" aria-labelledby="pointshistory-tab">
            <h2 class="accordion-header d-md-none" id="headingseven">
              <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseseven" aria-expanded="false" aria-controls="collapseseven"><span> Points History </span> <i class="fas fa-chevron-down "></i> </button>
            </h2>
            <div id="collapseseven" class="accordion-collapse collapse d-md-block" aria-labelledby="headingseven" data-bs-parent="#myTabContent">
               <div class="accordion-body">
                <!-- start order list -->
                <h3 class="text-center mb-4">Points {{ __('common.orders') }}</h3>
                <div class="order-list">
                  <div class="table-responsive">
                      @php
                      ...
                      @if(count($redeems)>0)
                        @foreach($redeems->reverse() as $redeem)
                          ...
                          <tr>
                              ...
                              <td>{{$redeem->product->title}}</td>
                              <td>{{$redeem->price}}</td>
                             
                              
                              <td>
                                {{ ucwords($redeem->status) }}
                              </td>
                              <td class="text-center">
                                  <a href="" class="default-button colorbtn1 btn-sm float-left mr-1" data-toggle="tooltip" title="view" data-placement="bottom">{{ __('common.order_detail') }}</a>
                                  <!-- <form method="POST" action="{{route('user.order.delete',[$order->id])}}">
                                    @csrf 
                                    @method('delete')
                                        <button class="btn btn-danger btn-sm dltBtn" data-id={{$order->id}} style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" data-placement="bottom" title="Delete"><i class="fas fa-trash-alt"></i></button>
              ...
              <div class="accordion-body ">
                ...
                <div class="order-list">
                  ...
                  <div class="text-center mt-3">
                                                <button type="submit" class="default-button me-auto"> Continue </button>
                         </div>
                </div>
...
document.getElementById('passwordForm').onsubmit = function () {
    ...
    if (newPassword !== confirmPassword) {
        document.getElementById('error').innerText = "{{ __('common.passwords_not_match') }}";
